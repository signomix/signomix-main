 ...
 */
public class H2CommandsDB extends H2EmbededDB implements SqlDBIface, ActuatorCommandsDBIface, Adapter {
    ...
    public Event getLastLog(String deviceEUI) throws ThingsDataException {
        String query = "select id,category,type,payload,createdat from commandslog where origin=? order by createdat limit 1";
        Event result = null;
        try (Connection conn = getConnection()) {
            PreparedStatement pst;
            pst = conn.prepareStatement(query);
