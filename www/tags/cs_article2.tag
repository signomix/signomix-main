<cs_article2>
    <article class={ opts.type }>
        ...
        <p if={ opts.type=='list' && opts.page && opts.uid }><a href={ article.link }>{ text.details[app.language] } &raquo;</a></p>
        <section ref='content'></section>
        <p if={ opts.type=='main' && opts.page }><a class="btn btn-outline-primary btn-sm" onclick="history.back()" role="button">&laquo; { text.back[app.language] }</a></p>
    </article>
